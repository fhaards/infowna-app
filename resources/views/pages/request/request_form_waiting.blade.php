...
@section('request-form')
    @foreach ($requests as $u)
        ...
        @endif
        <div
            class="w-full space-y-6 bg-white shadow-md border-t-4 border-{{ $rqs }}-300 rounded-lg p-4 sm:p-6 lg:p-8 dark:bg-gray-800 dark:border-gray-700">
            <div class="mb-2 p-3 text-center text-{{ $rqs }}-300 font-bold text-lg border-b">
                Submitted Form
            </div>
            <div class="flex sm:flex-row flex-col gap-3 pb-4 border-b">
                <div class="mb-2 sm:w-1/3">
                    ...
                    <p class="text-gray-600 font-medium">{{ $u->req_id }}</p>
                </div>
                <div class="mb-2 sm:w-1/3">
                    <label for="name" class="text-sm font-medium text-blue-800 block mb-2 dark:text-gray-300">
                        Status
            ...
            <div class="flex sm:flex-row flex-col gap-3 pb-4 border-b">
                <div class="mb-2 sm:w-1/3">
                    ...
                    <p class="text-gray-600 font-medium">{{ $u->name }}</p>
                </div>
            </div>
            <div class="flex sm:flex-row flex-col gap-3 pb-4 border-b">
                <div class="mb-2 sm:w-1/3">
            ...
            <div class="mb-2 flex sm:flex-row flex-col gap-3">
                <div class="mb-2 sm:w-1/3">
                    ...
                    <p class="text-gray-600 font-medium">{{ $u->phone }}</p>
                </div>
                <div class="mb-2 sm:w-2/3">
                    <label for="passport" class="text-sm font-medium text-blue-800 block mb-2 dark:text-gray-300">
                        Passport ID
                    </label>
                    <p class="text-gray-600 font-medium">{{ $u->passport_id }}</p>
                </div>
            </div>
            <div class="mb-2 w-full p-5">
                <div class="flex flex-row items-center justify-center">
                    <div class="w-48 sm:w-5/12 rounded-lg">
                        <img class="rounded-xl items-center w-full h-full object-cover"
                            src="{{ asset('storage/requests/passport/' . $u->passport_img) }}" alt="" />
                    </div>
                </div>
            </div>
            <div class="mb-2">
                ...
                <p class="text-gray-600 font-medium">{{ $u->address_indonesia }}</p>
            </div>
        </div>
    @endforeach
@endsection
